...
# MISSAO DO SCRIPT
#
# Faz o backup das configurações do servidor Apache2: /etc/apache2
#
...
function validaCompartilhamento(){
	...
	then
       ...
       data=$(date +%F_%H-%M)
       nome_arquivo="Apache2-conf_${data}.tgz"
	...
	then
		...
		exit 1
	       ...
	       #Realiza o backup
	       tar czf $destino_backup/$nome_arquivo $arquivos_backup && logApp INFO "Backup das configurações do Apache2 realizado com sucesso." || logApp ERROR "Falha ao realizar o backup das configurações do Apache2. Verifique: se possui permissao de escrita na pasta; se a pasta de destino esta mapeada; se o compartilhamento ainda existe; se o servidor esta conectado; se existe conexão de rede."
		   
		   #Lista os arquivos de backup
		   ls -lah /mnt/backup/app/app | grep Apache2
	fi
