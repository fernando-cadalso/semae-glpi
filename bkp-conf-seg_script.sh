...
# MISSAO DO SCRIPT
#
# Faz o backup das configurações de segurança e banco de dados: /etc/glpi
#
...
function validaCompartilhamento(){
	...
	then
       ...
       logApp INFO "Inicio do backup das configurações de segurança e banco de dados do GLPI..."

       #Origem dos arquivos
       arquivos_backup="/etc/glpi"
       ...
       data=$(date +%F_%H-%M)
       nome_arquivo="ConfigSeg-DB_${data}.tgz"
	...
	then
		...
		exit 1
	       ...
	       #Realiza o backup
	       tar czf $destino_backup/$nome_arquivo $arquivos_backup && logApp INFO "Backup das configurações de segurança e banco de dados do GLPI realizado com sucesso." || logApp ERROR "Falha ao realizar o backup das configurações de segurança e banco de dados do GLPI. Verifique: se possui permissao de escrita na pasta; se a pasta de destino esta mapeada; se o compartilhamento ainda existe; se o servidor esta conectado; se existe conexo de rede."
